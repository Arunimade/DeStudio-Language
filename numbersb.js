...
async function loadNumbers() {
    try {
        const response = await fetch('numbersb.json');
        bengaliNumbers = await response.json();
...
function updateNumber() {
    const number = bengaliNumbers[currentNumberIndex];
...
function nextNumber() {
    if (currentNumberIndex < bengaliNumbers.length - 1) {
        currentNumberIndex++;
        updateNumber();
    }
}
